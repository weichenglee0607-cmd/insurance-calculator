...
with st.sidebar:
    st.header("👤 客戶基本資料")
    # 使用 key 來確保輸入框的值與 session_state 強制綁定
    client_name = st.text_input("客戶姓名", value="新客戶", key="input_name")
    st.session_state['c_age'] = st.number_input("投保年齡", value=27)
...
# --- 模式 1：資料錄入 ---
if mode == "1. 資料錄入與對照":
    # 修正重點：直接使用變數 client_name，確保輸入什麼就顯示什麼
    st.header(f"📝 正在建立 {client_name} 的保單明細表")
...
# --- 模式 2：診斷報告 ---
    ...
    if df.empty:
        ...
        # 報告頁面同步更新
        st.header(f"📊 {client_name} {t_gender} ({st.session_state['c_age']}歲) 保障診斷報告")
